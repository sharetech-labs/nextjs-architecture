...
Next.js supports two Suspense strategies. Choose based on your UX goals:

- **Per-section Suspense** (shown below) — Each section streams in independently. Best when sections are independent or one is dramatically slower than others.
- **Page-level Suspense** — One skeleton covers the entire page; all data appears at once. Best for dashboards and pages with visually connected sections. See **`nextjs16-page-level-suspense`** for this pattern.

### Per-Section Streaming with Suspense
